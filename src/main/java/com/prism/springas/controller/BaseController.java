...
public class BaseController extends BaseBasicExtends {
    ...
    public ModelAndView excelExport(
            ...
            @RequestParam(value = "sortColumns", required = false, defaultValue = "") String sortColumns,
            @RequestParam(value = "sortTypes", required = false, defaultValue = "") String sortTypes,
            @RequestParam(value = "excelName", required = false, defaultValue = "") String excelName,
            @RequestParam(value = "cellName", required = false, defaultValue = "") String cellName,
            @RequestParam(value = "cellParse", required = false, defaultValue = "") String cellParse,
            @RequestParam(value = "data", required = false, defaultValue = "{}") String data) throws Exception {
        sqlEngine sqlEngine = super.jsonToSqlEngine(0, data);
        if (sqlEngine == null) {
            return null;
            ...
            //获取主表NAME
            String[] tableArray = selTable.split(",");
            sqlEngine.getParamEx("excelName", excelName);
            sqlEngine.getParamEx("cellName", cellName);
            sqlEngine.getParamEx("cellParseName", cellParse);
            //通过Prism棱镜获取数据返回结果
            return (ModelAndView) super.invokeSchemaMethod(0, tableArray[0],
                    "excelExport", sqlEngine);
        }
    }
    ...
    public String eCharsBase(
        ...
        if (sqlEngine == null) {
            ...
            if (reObj != null) {
                eCharsEngine eCharsEngine = new eCharsEngine(eCharsType.contains("bar") ? 1 : 0, (List<BasePage>) reObj);
                if (eCharsType.equals("line") || eCharsType.equals("bar")) {
                    ...
                    //饼图
                    reObj = eCharsEngine.basicPie(legendCol, seriesDataCol).toParsePie(title, seriesName).getEcharsData();
                } else if (eCharsType.equals("doughnut")) {
                    //环形图
