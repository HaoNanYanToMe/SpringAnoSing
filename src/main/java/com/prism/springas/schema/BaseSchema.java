...
public class BaseSchema implements Serializable{
    ...
    @Autowired
    ExcelImportTool excelImportTool;
     ...
     * @return
     */
    public ModelAndView excelExport(String tableName, sqlEngine sqlEngine){
        List<BasePage> listData = baseApi.selectPage(tableName,sqlEngine);
        return  excelImportTool.importExcel(listData,sqlEngine.getBaseParam("cellName").toString(),
                sqlEngine.getBaseParam("cellParseName").toString(),sqlEngine.getBaseParam("excelName").toString());
    }
