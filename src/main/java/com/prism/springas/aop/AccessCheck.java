...
public class AccessCheck {
    ...
    public String loginProcess(ProceedingJoinPoint point) throws Exception {
        ...
        if (user != null){
            ...
            String currentUser =  jedisUtil.get("tk:"+oAuthToken.GetCookie("UUID"))+"";
            //TOKEN鉴权
    ...
    private boolean checkCurrentRole(String signatrue, String tableName, String fucName, String rid) throws Exception {
        Object roleMenusCache = jedisUtil.getBaseList("ro:"+rid);
        ...
        if (roleMenusCache != null) {
            ...
            //权限缓存-1小时
            jedisUtil.setObject("ro:"+rid, roleMenus, 3600);
        }
        boolean re = false;
        if (roleMenus.size() > 0) {
            int add = 0;
